...
class OutputService {
    public function __construct(
        ...
        $whiteBlink = new OutputFormatterStyle('white', options: ['bold', 'blink']);
        ...
        $redBlink = new OutputFormatterStyle('red', options: ['bold', 'blink']);
        ...
        $greenBlink = new OutputFormatterStyle('green', options: ['bold', 'blink']);
        ...
        $yellowBlink = new OutputFormatterStyle('yellow', options: ['bold', 'blink']);
        ...
        $blueBlink = new OutputFormatterStyle('blue', options: ['bold', 'blink']);
        ...
        $magentaBlink = new OutputFormatterStyle('magenta', options: ['bold', 'blink']);
        ...
        $cyanBlink = new OutputFormatterStyle('cyan', options: ['bold', 'blink']);
        ...
        $blackBlink = new OutputFormatterStyle('black', options: ['bold', 'blink']);
    ...
    public function outputRenderTask(): self
    {
        ...
        $this->getSymfonyStyle()->text(
            ...
            . ($taskArray['duration'] ? "<yellow>" . $taskArray['duration'] . "</yellow> " : "")
            . ($taskArray['remaining'] ? "🎯" . $taskArray['remaining'] . " " : "")
    ...
    public function outputRenderTasks(): self {
        ...
        foreach ($tasks ?? [] as $task):
            ...
            if (!empty($endDate)):
                $endDate = (new \DateTime(timezone: $this->getConfig()->getDateTimeZone()))->setTimestamp($endDate);
            else:
                $endDate = $startDate;
            endif;
            ...
            foreach ($tables as $index => $tasks):
                ...
                foreach ($tasks as $task):
                    ...
                    $this->getSymfonyStyle()->text(
                        ...
                        . ($task['duration'] ? "<yellow>" . $task['duration'] . "</yellow> " : "")
    ...
    public function renderCommit(Commit $commit, int $index, int $totalSeconds): self {
        $this->getSymfonyStyle()->text(
            " 🍺 "
            . "<yellow>" . $commit->getDuration(totalSeconds: $totalSeconds) . "</yellow>"
            . ($commit->getDuration() ? " <green>(+" . $commit->getDuration() . ")</green>" : "")
